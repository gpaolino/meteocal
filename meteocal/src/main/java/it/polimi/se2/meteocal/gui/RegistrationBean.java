/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package it.polimi.se2.meteocal.gui;

import it.polimi.se2.meteocal.boundary.UserManager;
import it.polimi.se2.meteocal.entity.AdminSettings;
import it.polimi.se2.meteocal.entity.Group;
import it.polimi.se2.meteocal.entity.Location;
import it.polimi.se2.meteocal.entity.User;
import it.polimi.se2.meteocal.entity.WForecast;
import it.polimi.se2.meteocal.meteoInfo.WeatherForecast;
import java.sql.SQLException;
import java.text.ParseException;
import javax.ejb.EJB;
import javax.faces.bean.ManagedBean;
import javax.faces.bean.RequestScoped;
import org.json.JSONException;

/**
 *
 * @author alessandronegrini
 */

@ManagedBean(name="registrationBean")
@RequestScoped
public class RegistrationBean {

    @EJB
    private UserManager um;
    private User user;
    private Location location;
    private WForecast wf;
    private String avatarString;
    private AdminSettings aS=new AdminSettings();
    
    public String getAvatarString() {
        return avatarString;
    }

    public void setAvatarString(String avatarString) {
        this.avatarString = avatarString;
    }

    public UserManager getUm() {
        return um;
    }

    public void setUm(UserManager um) {
        this.um = um;
    }

    public Location getLocation() {
            if (location == null) {
            location = new Location();
        }
        return location;
    }

    public void setLocation(Location location) {
        this.location = location;
    }
    
    public RegistrationBean() {
    }

    public User getUser() {
        if (user == null) {
            user = new User();
        }
        return user;
    }

    public void setUser(User user) {
        this.user = user;
    }
    
    /**
     * It Registers user to the database after having checked if the user is a normal user or an administrator
     * @return url to which user will be redirected (index page)
     * @throws SQLException
     * @throws JSONException 
     */
    public String register() throws SQLException, JSONException, ParseException {
        
        if(um.find(user.getEmail())!=null && "NotRegisteredUser".equals(um.find(user.getEmail()).getType())){
            this.alreadyExists();
            return "index?faces-redirect=true";
        }
        
        if("".equals(user.getAvatarString())){
            user.setAvatarString("data:image/png;base64,");
        }
        user.setHasPublicCalendar(Boolean.FALSE);       
        user.setType("Normal User");
        user.setResPassTime("0");
        wf=this.getWeather();
        user.setWeather(wf);
        location.setNumber(null);
        location.setStreet(null);
        user.setLocation(location);
        System.out.println("User added to DB: "+ user.getName() +" "+user.getEmail());
        System.out.println("Location Details: "+ location.getCity() +" "+location.getCoordinates()); 
        um.save(user,location,wf);
        return "index?faces-redirect=true";
    }

    /**
     * It returns the weather including temperature and humidity percentage
     * @return the object WForecast with weather condition, temperature and humidity
     * @throws JSONException 
     */
    private WForecast getWeather() throws JSONException{
        
        WForecast WF = new WForecast();
        WeatherForecast wf=new WeatherForecast(this.location.getCity());
        Float temperature = wf.getTemperature();
        Float humidity=wf.getHumidity();
        String weather=wf.getWeather();
        
        WF.setHumidity(humidity);
        WF.setTemperature(temperature);
        WF.setWeather(weather);
        
        return WF;
    }
    
    private void alreadyExists() throws JSONException, ParseException{      
        User u = um.find(user.getEmail());
        user.setAvatarString(u.getAvatarString());
        user.setLocation(u.getLocation());
        user.setWeather(u.getWeather());
        user.setGroupName(Group.USERS);
        user.setHasPublicCalendar(Boolean.FALSE);
        user.setResPassTime("0");
        user.setType("Normal User");
        um.edit(user);     
    }
    
}
